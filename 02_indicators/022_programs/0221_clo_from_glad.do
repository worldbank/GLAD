...
local dofile_info = "last modified by Yi Ning Wong in March 9, 2022"
*==============================================================================*
  ...
  if _rc != 0 {
    ...
    * If the file does not exist or overwrite_files local is set to one, continue
    if (_rc == 601) | ($overwrite_files_GLAD_02) {
	  ...
	  *Comment_AR: /// changed filename local in dlw command: from `v01_input_file' to `wrk_input_file'
	  ...
	  * this code now creates bowth the SDG 4.1.1 indicator and its complement (BMP)
	  ...
	  if "`assessment'" == "AMPLB" {
		  ...
		  groups(countrycode idgrade) subgroups(male urban)         ///
