...
quietly {
  ...
  *---------------------------------------------------------------------------
  ...
  if (_rc == 601) | (`overwrite_files') {
    ...
    * 1) Open all rawdata, lower case vars, save in temp_dir
    *---------------------------------------------------------------------------
    foreach prefix in 2010 2012_1_2 2012_3_6 {
      if "`year'" == "All" {
    ...
    *<_idlearner_>
    bysort year grade idschool: gen idlearner = _n  // Given that there is no student id but each obs is one student
    ...
    *<_score_assessment_subject_pv_>
    egen aux_score_egra_read = rowtotal(sophy*)
    gen  score_egra_read_pcnt = (aux_score_egra_read/5)*100
    label var score_egra_read_pcnt	"Egra score for read (pcnt)"
    *</_score_assessment_subject_pv_>
    ...
    *<_learner_weight_>
    gen learner_weight = 1 // PLACEHOLDER
    ...
    * 4) ESCS and other calculations (by Aroob, from Feb 2019)
    *---------------------------------------------------------------------------
    // There was never a ESCS for this file in HAD, so this section is TBD
    gen escs = .a
    ...
    *** SEGMENT NOT NEEDED FOR EGRA, BY DEFAULT A SINGLE COUNTRY ***
    ...
    char _dta[language_test]                "Khmer"
    char _dta[language_instruction]         "Khmer"
    char _dta[nationally_representative]    "1"
    char _dta[regionally_representative]    "1"
    char _dta[has_learner_weights]			    "0"
