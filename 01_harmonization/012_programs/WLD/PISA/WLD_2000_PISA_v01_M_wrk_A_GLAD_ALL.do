  ...
  if (_rc == 601) | (`overwrite_files') {
    ...
    *---------------------------------------------------------------------------
    ...
    foreach file in ESCS_PISA2000 intscho math read science {
      ...
      rename *, lower
      * Though we avoid renaming variables, this is needed before the merge
      if inlist("`file'", "math", "read", "science") {
	    rename (w_fstuwt w_fstr*) (w_fstuwt_`file2' w_fstr*_`file2')
      }
      compress
    ...
    *---------------------------------------------------------------------------
	...
	local temp_dir ${clone}/01_harmonization/013_outputs\TEMP\TEMP_WLD_2000_PISA
    ...
    *<_idgrade_>
    clonevar idgrade = st02q01
    mvdecode idgrade, mv(96 = .z \ 97 = .a \ 98 = .b \ 99 = .c)
    label copy ST02Q01 idgrade
    ...
    *<_age_>
    rename age age_o
    recode age_o (997 999 = .z), gen(age)
    replace age = age/12
    label var age "Learner age at time of assessment"
    *</_age_>

    *<_urban_>
    recode sc01q01 (1 = 0 "Rural") (2/6 = 1 "Urban") (7 = .a "Not Administered") (8 = .b "Invalid") (9 = .c "No Response"), gen(urban)
    label var urban "School is located in urban/rural area"
    *</_urban_>

    *<_urban_o_>
    decode sc01q01, gen(urban_o)
    ...
    *<_male_>
    recode st03q01 (2 = 1 "Male") (1 = 0 "Female") (7 = .a "Not Administered") (8 = .b "Invalid") (9 = .c "No Response"), gen(male)
    ...
    *</_male_>
    ...
    /* NOTE: weights are by subject */

    *<_learner_weight_>
    foreach subject in read math scie {
      clonevar  learner_weight_`subject'  = w_fstuwt_`subject'
      label var learner_weight_`subject' "Total learner weight (`subject')"
    }
    ...
    label var learner_weight "Total learner weight / using learner_weight_read as a proxy for overall weight"
    *</_learner_weight_>

    *<_weight_replicateN_>
    foreach subject in read math scie {
      forvalues i = 1(1)80 {
        clonevar  weight_replicate`i'_`subject' = w_fstr`i'_`subject'
        label var weight_replicate`i'_`subject' "Replicate weight `i' (`subject')"
      }
    }
    *</_weight_replicateN_>
    ...
    *</_escs_quintile_>
    ...
    // PISA Vars:
    local pisavars "city native language school_type school_type_o"
    ...
    *<_school_type_o_> - original school type variable
    ...
    replace school_type_o = -97 if inlist(sc03q01,7)
